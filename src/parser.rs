...
fn expect(lexer: &mut Lexer, tok: Token) -> Result<(), ParseError> {
    if lexer.tok == tok {
        lexer.next();
        Ok(())
    } else {
...
fn expect_id(lexer: &mut Lexer) -> Result<Name, ParseError> {
    if let Token::Id(string) = &lexer.tok {
        let result = Ok(Name::new(string.clone()));
        lexer.next();
        result
    } else {
