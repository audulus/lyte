...
fn expect(tok: Token, cx: &mut ParseContext) {
    if cx.lexer.tok == tok {
        ...
        let message = format!("expected {:?}, got {:?}", tok, cx.lexer.tok);
        cx.err(message)
    }
}

fn expect_id(cx: &mut ParseContext) -> Name {
    if let Token::Id(string) = &cx.lexer.tok {
        let result = Name::new(string.clone());
        cx.next();
        result
    } else {
        let message = format!("expected identifier, got {:?}", cx.lexer.tok);
        println!("{:?}", message);
        cx.err(message);
        Name::new("".to_string())
    }
...
fn parse_basic_type(typevars: &[Name], cx: &mut ParseContext) -> TypeID {
    mk_type(match &cx.lexer.tok {
        ...
        Token::Lbracket => {
            ...
            if cx.lexer.tok == Token::Semi {
                cx.next();
                if let Token::Integer(n) = cx.lexer.tok.clone() {
                    cx.next();
...
fn parse_type(typevars: &[Name], cx: &mut ParseContext) -> TypeID {
    ...
    while cx.lexer.tok == Token::Arrow {
        cx.next();
...
fn parse_paramlist(
    ...
    loop {
        if let Token::Id(name) = &cx.lexer.tok {
            let name = name.clone();
            cx.next();

            // In the case of lambdas, we can omit the types.
            let ty = if cx.lexer.tok == Token::Colon {
                cx.next();
...
fn parse_lambda(
    ...
    if cx.lexer.tok == Token::Pipe {
        ...
        arena.add(Expr::Lambda { params, body }, cx.lexer.loc)
    } else {
...
fn parse_if(
    ...
    let els = if cx.lexer.tok == Token::Else {
        cx.next();
        if cx.lexer.tok == Token::If {
            cx.next();
    ...
    arena.add(Expr::If(cond, then, els), cx.lexer.loc)
}
...
fn parse_assign(
    ...
    while cx.lexer.tok == Token::Assign {
        let t = cx.lexer.tok.clone();
        cx.next();
        let rhs = parse_eq(arena, typevars, cx);

        lhs = arena.add(binop(&t, lhs, rhs), cx.lexer.loc)
    }
...
fn parse_eq(
    ...
    while cx.lexer.tok == Token::Equal || cx.lexer.tok == Token::NotEqual {
        let t = cx.lexer.tok.clone();
        cx.next();
...
fn parse_rel(
    ...
    while cx.lexer.tok == Token::Leq
        || cx.lexer.tok == Token::Geq
        || cx.lexer.tok == Token::Less
        || cx.lexer.tok == Token::Greater
    {
        let t = cx.lexer.tok.clone();
        cx.next();
...
fn parse_sum(
    ...
    while cx.lexer.tok == Token::Plus || cx.lexer.tok == Token::Minus {
        let t = cx.lexer.tok.clone();
        cx.next();
...
fn parse_term(
    ...
    while cx.lexer.tok == Token::Mult || cx.lexer.tok == Token::Div {
        let t = cx.lexer.tok.clone();
        cx.next();
...
fn parse_exp(
    ...
    while cx.lexer.tok == Token::Power {
        cx.next();

        let rhs = parse_factor(arena, typevars, cx);

        lhs = arena.add(binop(&Token::Power, lhs, rhs), cx.lexer.loc)
    }
...
fn parse_factor(
    ...
    match &cx.lexer.tok {
        Token::Minus => {
            ...
            arena.add(Expr::Unop(e), cx.lexer.loc)
        }
        Token::Plus => {
            cx.next();
...
fn parse_postfix(
    ...
    loop {
        match cx.lexer.tok {
            Token::Lparen => {
                cx.next();
                let args = if cx.lexer.tok != Token::Rparen {
                ...
                e = arena.add(Expr::Call(e, args), cx.lexer.loc);
            }
            Token::Colon => {
                cx.next();
                let t = parse_type(typevars, cx);
                e = arena.add(Expr::AsTy(e, t), cx.lexer.loc);
            }
            Token::Lbracket => {
                cx.next();
                let idx = parse_expr(arena, typevars, cx);
                expect(Token::Rbracket, cx);
                e = arena.add(Expr::ArrayIndex(e, idx), cx.lexer.loc);
            }
            Token::Dot => {
                cx.next();
                let field = expect_id(cx);
                e = arena.add(Expr::Field(e, field), cx.lexer.loc);
            }
            Token::As => {
                cx.next();
                let ty = parse_type(typevars, cx);
                e = arena.add(Expr::AsTy(e, ty), cx.lexer.loc);
            }
            _ => {
                return e;
            }
...
fn parse_atom(
    ...
    match &cx.lexer.tok {
        Token::Id(id) => {
            ...
            cx.next();
            arena.add(e, cx.lexer.loc)
        }
        Token::True => {
            cx.next();
            arena.add(Expr::True, cx.lexer.loc)
        }
        Token::False => {
            cx.next();
            arena.add(Expr::False, cx.lexer.loc)
        }
        Token::Dot => {
            cx.next();
            let name = expect_id(cx);
            arena.add(Expr::Enum(name), cx.lexer.loc)
        }
        Token::Integer(x) => {
            let e = Expr::Int(*x);
            cx.next();
        ...
        _ => {
            ...
            arena.add(Expr::Tuple(vec![]), cx.lexer.loc)
        }
    }
}
...
fn parse_array_literal(
    ...
    while cx.lexer.tok != Token::Rbracket {
        r.push(parse_lambda(arena, typevars, cx));

        if cx.lexer.tok == Token::Comma {
            cx.next();
        } else if cx.lexer.tok == Token::Semi {
            cx.next();
            let count = parse_expr(arena, typevars, cx);
            let e = Expr::Array(r[0], count);
            expect(Token::Rbracket, cx);
            return arena.add(e, cx.lexer.loc);
        } else if cx.lexer.tok == Token::Rbracket {
            break;
        } else {
    ...
    arena.add(Expr::ArrayLiteral(r), cx.lexer.loc)
}
...
fn parse_stmt(
    ...
    match cx.lexer.tok.clone() {
        Token::Var => {
            cx.next();
            let name = expect_id(cx);

            if cx.lexer.tok == Token::Assign {
                cx.next();
                let e = parse_lambda(arena, typevars, cx);
                arena.add(Expr::Var(name, Some(e), None), cx.lexer.loc)
            } else if cx.lexer.tok == Token::Colon {
                cx.next();
                let t = parse_type(typevars, cx);
                arena.add(Expr::Var(name, None, Some(t)), cx.lexer.loc)
            } else {
                cx.err(String::from("expected assignment or type"));
                arena.add(Expr::Var(name, None, None), cx.lexer.loc)
            }
        }
        Token::Let => {
            cx.next();
            ...
            if cx.lexer.tok == Token::Assign {
                cx.next();
                let e = parse_lambda(arena, typevars, cx);
                arena.add(Expr::Let(name, e, None), cx.lexer.loc)
            } else {
                cx.err(String::from("expected assignment or type"));
                arena.add(Expr::Error, cx.lexer.loc)
            }
        }
        Token::Arena => {
            cx.next();
            let e = parse_block(arena, typevars, cx);
            arena.add(Expr::Arena(e), cx.lexer.loc)
        }
        Token::While => {
            cx.next();
            let cond = parse_expr(arena, typevars, cx);
            let body = parse_block(arena, typevars, cx);
            arena.add(Expr::While(cond, body), cx.lexer.loc)
        }
        Token::Return => {
            cx.next();
            let e = parse_expr(arena, typevars, cx);
            arena.add(Expr::Return(e), cx.lexer.loc)
        }
        Token::For => {
            cx.next();
            ...
            let body = parse_block(arena, typevars, cx);
            arena.add(Expr::For { var, start, end, body }, cx.lexer.loc)
        }
...
fn parse_block(
    ...
    loop {
        ...
        if cx.lexer.tok == Token::Rbrace {
            break;
        }

        r.push(parse_stmt(arena, typevars, cx));

        if cx.lexer.tok != Token::Endl {
            break;
        }
    ...
    arena.add(Expr::Block(r), cx.lexer.loc)
}
...
fn parse_fieldlist(
    ...
    while cx.lexer.tok != Token::Rbrace {
        skip_newlines(cx.lexer);
...
fn parse_func_decl(name: Name, cx: &mut ParseContext) -> FuncDecl {
    ...
    if cx.lexer.tok == Token::Where {
        cx.next();

        while !token_in(&cx.lexer.tok, &[Token::Lbrace, Token::Rbrace, Token::Endl]) {
...
fn parse_decl(cx: &mut ParseContext) -> Option<Decl> {
    Some(match cx.lexer.tok.clone() {
        ...
        _ => {
            cx.err(format!("Expected declaration, got {:?}", cx.lexer.tok));
            cx.next();
            return None;
        }
    })
...
pub fn parse_program(lexer: &mut Lexer, errors: &mut Vec<ParseError>) -> Vec<Decl> {
    ...
    let mut cx = ParseContext { lexer, errors };

    skip_newlines(cx.lexer);

    while cx.lexer.tok != Token::End {
...
mod tests {
    ...
    fn type_parser(string: &str) -> TypeID {
        ...
        let mut errors = vec![];
        let mut cx = ParseContext { lexer: &mut lexer, errors: &mut errors };
        cx.next();
    ...
    fn parse_fn<T: std::fmt::Debug>(
        ...
        let mut cx = ParseContext { lexer: &mut lexer, errors: errors };
        cx.next();
    ...
    fn test_parse_program() {
        test_strings(
            |_, _, cx| parse_program(cx.lexer, cx.errors),
            &["", "\n", "\nf()", "f(){} g(){}", "f(){}\n g(){}", "f()\n{}"],
        );
    }
}
